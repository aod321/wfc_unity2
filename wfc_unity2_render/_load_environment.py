...
def _create_channel_and_connection(port, host="0.0.0.0"):
...
def _connect_to_environment(port, create_world_settings, join_world_settings, host="0.0.0.0"):
  """Helper function for connecting to a running dm_env_rpc environment."""
  channel, connection = _create_channel_and_connection(port, host=host)
...
def load_from_disk(path, settings):
  ...
  return _DemoTasksProcessEnv(
      _connect_to_environment(port, settings.create_world_settings,settings.join_world_settings, host="127.0.0.1"), _TASK_OBSERVATIONS,
