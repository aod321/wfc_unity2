...
def DiGraphStats(DiGraph):
    ...
    return np.mean(all_pair_length), no_path_pair_rate
    
    # # square_clustering, the result is the probability that two neighbors of node v share a common neighbor different from v. 
    # square_clustering = nx.square_clustering(DiGraph)
    # mean_square_clustering = np.mean(list(square_clustering.values()))
    # print("mean square clustering: ", mean_square_clustering)
    # all_pairs_node_connectivity = nx.all_pairs_node_connectivity(DiGraph)
    # all_pairs_node_connectivity_list = []
    # for key in all_pairs_node_connectivity:
    #     all_pairs_node_connectivity_list.extend(list(all_pairs_node_connectivity[key].values()))
    # print("mean all pairs node connectivity: ", np.mean(all_pairs_node_connectivity_list))
    # average_neighbor_degree = nx.average_neighbor_degree(DiGraph, source="in+out", target="in+out")
    # mean_average_neighbor_degree = np.mean(list(average_neighbor_degree.values()))
    # print("mean average neighbor degree: ", mean_average_neighbor_degree)
    # average_degree_connectivity = nx.average_degree_connectivity(DiGraph)
    # mean_average_degree_connectivity = np.mean(list(average_degree_connectivity.values()))
    # print("mean average degree connectivity: ", mean_average_degree_connectivity)
    # try:
    #     find_cycle = nx.find_cycle(DiGraph)
    #     length_cycle = len(find_cycle)
    #     print("length of cycle: ", length_cycle)
    # except:
    #     pass
